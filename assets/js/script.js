...
let compTurn;
/* boolean true/false to keep track of 
...
function gameTurn() {
  ...
  init()
  if (flash === round) {
    /* if the numbers of time the lights have have flashed 
  ...
  if (compTurn) {
    /* the thing in the parenthesis is the condition and then 
        you are going to run the thing in the cury brackets */
    clearColor();
    /* setTimeout is very similar to setInterval which repeats something over and over, 
...
// based on free Code Camp Simon Game tutorial: https://youtu.be/n_ec3eowFLQ
